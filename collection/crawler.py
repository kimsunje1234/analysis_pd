...
def crawling_tourspot_visitor(
        ...
        fetch=True):
    results = []
    filename = '%s/%s_tourspot_%s_%s.json' % (restore_directory, district, start_year, end_year)
...
def crawling_foreign_visitor(
    ...
    results = []
    filename = '%s/%s(%s)_foreignvisitor_%s_%s.json' % (restore_directory, country[0], country[1], start_year, end_year)
